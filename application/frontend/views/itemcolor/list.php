    ...
    <div class="span12">
        <table width="100%" cellpadding="5" cellspacing="5" border="0" class="table table-striped table-bordered table-hover dataTable" id="pagelist_center">
            ...
            <tbody>
                ...
                {
                    foreach($rsColors as $arrRecord)
                    {
                        $strEditLink	=	"index.php?c=itemcolor&m=Add&action=E&id=".$arrRecord['item_color_id'];
                        echo '<tr>';
						echo '<td><input type="checkbox" name="chk_lst_list1[]" id="chk_lst_'.$arrRecord['item_color_id'].'" value="'.$arrRecord['item_color_id'].'" /><span class="lbl"></span></td>';
						...
						echo '<td>'. $arrRecord['item_color_name'] .'</td>';
                        echo '<td>'. $arrRecord['item_color_code'] .'</td>';
...
{
    window.location.href = 'index.php?c=itemcolor&m=Add&action=A';
}
...
function DeleteRow()
{
	...
	else
	{
		...
		if(responce==true)
		{
			$('#frm_list_record').attr('action','index.php?c=itemcolor&m=delete');
